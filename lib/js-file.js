...
var JsFile = function(filename, source, tree, options) {
    ...
    this._markCommentTokens(this._tree.comments);
...
JsFile.prototype = {
     ...
     * @returns {Object|undefined}
     */
    getPrevToken: function(token, includeComments) {
        ...
        if (index >= 0) {
            if (includeComments) {
                ...
                do {
                    if (!this._tokens[index]._isComment) {
                        return this._tokens[index];
                    }
                    index--;
                } while (index >= 0);
     ...
     * @returns {Object|undefined}
     */
    getNextToken: function(token, includeComments) {
        var index = token._tokenIndex + 1;
    ...
    getCommentAfterToken: function(token) {
        var nextToken = this.getNextToken(token, true);
        return nextToken && nextToken._isComment ? nextToken : undefined;
    },
    ...
    getCommentBeforeToken: function(token) {
        var prevToken = this.getPrevToken(token, true);
        return prevToken && prevToken._isComment ? prevToken : undefined;
    },
    ...
    _buildTokenList: function(codeTokens, commentTokens) {
        ...
        while (codeQueue.length > 0 || commentQueue.length > 0) {
            if (codeQueue.length > 0) {
                if (commentQueue.length > 0 && commentQueue[0].range[0] < codeQueue[0].range[0]) {
                    result.push(commentQueue.shift());
                } else {
    ...
    /**
     * Adds `_isComment` flat to the comment-tokens.
     *
     * @param {Object[]} commentTokens
     * @private
     */
    _markCommentTokens: function(commentTokens) {
        // For-loop for maximal speed.
        for (var i = 0; i < commentTokens.length; i++) {
            commentTokens[i]._isComment = true;
        }
    },
    ...
    _applyWhitespaceData: function(tokens, source) {
        ...
        for (var i = 0; i < tokens.length; i++) {
            ...
            token._whitespaceBefore = whitespace;
            if (prevToken) {
                prevToken._whitespaceAfter = whitespace;
            }
        ...
        if (prevToken) {
            prevToken._whitespaceAfter = source.substring(prevToken.range[1], source.length);
        }
    },
...
JsFile.parse = function(source, esprima, esprimaOptions) {
    ...
    if (hashbang) {
        var hashbangComment = tree.comments[0].value;
        tree.comments = tree.comments.slice(1);
        tree.hashbang = hashbangComment;
    }
